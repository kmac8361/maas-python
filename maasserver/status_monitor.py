...
def mark_nodes_failed_after_expiring():
    ...
    for node in expired_nodes:
        if node.status == NODE_STATUS.DEPLOYING and "WIN" in node.hostname:
            node.end_deployment()
        else:
            maaslog.error("mark_nodes_failed_after_expiring.. Node: %s: Operation '%s' timed out after %s minutes." % (
                node.hostname,
