...
class Node(CleanSave, TimestampedModel):
    ...
    def _start_deployment(self):
        ...
        maaslog.error(
                "DEBUG: Starting deployment for node: %s",
                self.hostname)
    ...
    def end_deployment(self):
        ...
        maaslog.error(
                "DEBUG: Ending deployment for node: %s  Marking Deployed",
                self.hostname)
    ...
    def get_effective_power_parameters(self):
        ...
        #maaslog.error(
        #        "DEBUG: Get power params for node: %s  Boot mode %s",
        #        self.hostname, power_params['boot_mode'])
        return power_params
    ...
    def set_netboot(self, on=True):
        ...
        maaslog.info("%s: Turning on netboot for node", self.hostname)
        self.netboot = on
        self.save()
    ...
    def mark_failed(
        ...
        maaslog.error(
                "DEBUG: Marking node: %s as failed... Comment %s",
                self.hostname, comment)
    ...
    def update_power_state(self, power_state):
        ...
        #maaslog.error(
        #        "DEBUG: Update power state of node: %s Power state: %s  Deploy Status: %s",
        #        self.hostname, power_state, NODE_STATUS_CHOICES_DICT[self.status])
    ...
    def _start(
        ...
        if deployment_timeout is not None:
            maaslog.error(
                "DEBUG: Deployment time for node: %s",
                self.hostname)
            d.addCallback(
                callOutToDatabase, Node._set_status_expires,
