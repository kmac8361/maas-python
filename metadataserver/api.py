...
class VersionIndexHandler(MetadataViewHandler):
    ...
    def _process_deploying(self, node, request, status):
        ...
        if status == SIGNAL_STATUS.FAILED:
            logger.info("process_deploying... mark_failed - check install log")
            node.mark_failed(
    ...
    def read(self, request, version, mac=None):
        ...
        try:
            ...
            if node.status == NODE_STATUS.DEPLOYING:
                logger.info("UserDataHandler: Invoking end deployment. Node - %s" % node.hostname)
                if (mac != None):
                    logger.info("UserDataHandler: Invoking end deployment. Mac - %s" % mac)
                node.end_deployment()
                logger.info("UserDataHandler: Completed call from end deployment. Node - %s" % node.hostname)
            # If this node is supposed to be powered off, serve the
            # 'poweroff' userdata.
            if node.get_boot_purpose() == 'poweroff':
                #logger.info("UserDataHandler: Invoking generate user data for poweroff. Node - %s" % node.hostname)
                user_data = generate_user_data_for_poweroff(node=node)
            else:
                #logger.info("UserDataHandler: Calling get user data. Node - %s" % node.hostname)
            ...
            logger.info(
                "UserDataHandler: No user data registered for node named %s" % node.hostname)
...
def add_file_to_tar(tar, path, content, mtime, permission=0o755):
    ...
    # Modification time defaults to Epoch, which elicits annoying:1
