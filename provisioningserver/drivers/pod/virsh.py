...
class VirshSSH(pexpect.spawn):
    ...
    def attach_local_volume(self, domain, pool, volume, device):
        ...
        maaslog.error("DEBUG Calling attach-disk... Domain=%s Device=%s VolPath=%s Pool=%s", domain, device, vol_path, pool)
        if "WIN" in domain:
            self.run([
                'attach-disk', domain, vol_path, device,
                '--targetbus', "ide", '--sourcetype', 'file', '--config'])
        else:
            self.run([
                'attach-disk', domain, vol_path, device,
    ...
    def attach_bridge(self, domain, bridge):
        """Attach new bridge interface on `domain` to `bridge`."""
        self.run([
            'attach-interface', domain, 'bridge', bridge,
            '--model', 'virtio', '--config'])
    ...
    def get_block_name_from_idx(self, domain, idx):
    ...
    def create_domain(self, request):
        ...
        if "WIN" in request.hostname:
            domain_xml = WIN_DOM_TEMPLATE.format(**domain_params)
        else:
        ...
        for idx, (pool, volume) in enumerate(created_disks):
            block_name = self.get_block_name_from_idx(request.hostname,idx)
