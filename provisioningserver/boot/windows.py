...
def request_node_info_by_mac_address(mac_address):
    ...
    """
    maaslog.error("Windows: Request node info by MAC address.")
...
class Bcd:
    ...
    """
    maaslog.error("Windows: Bcd - allow modification of load options.")
    ...
    def set_load_options(self, value):
        ...
        maaslog.error("Windows: Set load options.")
...
class WindowsPXEBootMethod(BootMethod):
    ...
    def get_node_info(self):
        ...
        maaslog.error("WindowsPXEBoot: Get node info...")
    ...
    def match_path(self, backend, path):
        ...
        """
        #maaslog.error("WindowsPXEBoot: Match path...")
        ...
        if path == 'pxelinux.0':
            #maaslog.error("WindowsPXEBoot: Match path pxelinux.0...")
        ...
        # Fix the paths for the other static files, Windows requests.
            ...
            #maaslog.error("WindowsPXEBoot: Match path static files...")
            returnValue({
                'mac': get_remote_mac(),
    ...
    def compose_preseed_url(self, url):
        ...
        """
        maaslog.error("WindowsPXEBoot: compose_preseed_url. URL: %s", url)
        url = url.replace('/', '\\')
    ...
    def compose_bcd(self, kernel_params, local_host):
        ...
        maaslog.error("WindowsPXEBoot: compose_bcd. LoadOptions: %s", loadoptions)
    ...
    def output_static(self, kernel_params, path):
        ...
        maaslog.error("WindowsPXEBoot: output_static. path: %s", path)
